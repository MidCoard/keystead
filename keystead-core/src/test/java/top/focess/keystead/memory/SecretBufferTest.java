...
class SecretBufferTest {
    ...
    void toStringIsAlwaysRedacted() {
        try (SecretBuffer buffer = SecretBuffer.fromChars(new char[]{'s', 'e', 'c', 'r', 'e', 't'})) {
    ...
    void closeWipesOwnedCharactersAndRejectsFutureAccess() {
        ...
        assertThrows(SecretDestroyedException.class, () -> buffer.copyChars(chars -> fail("closed buffer should not be readable")));
    ...
    void callbackReceivesTemporaryCopyForBytes() {
        try (SecretBuffer buffer = SecretBuffer.fromUtf8("token".getBytes(StandardCharsets.UTF_8))) {
            AtomicReference<byte[]> seen = new AtomicReference<>();

            buffer.copyBytes(bytes -> {
                seen.set(bytes.clone());
                bytes[0] = 'X';
            });

            assertArrayEquals("token".getBytes(StandardCharsets.UTF_8), seen.get());
            buffer.copyBytes(bytes -> assertArrayEquals("token".getBytes(StandardCharsets.UTF_8), bytes));
        }
    }
